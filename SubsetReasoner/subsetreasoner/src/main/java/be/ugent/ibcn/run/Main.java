...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < setup.length; i++) {
			setupOnts[i] = manager.loadOntologyFromOntologyDocument(classloader.getResourceAsStream(setup[i] + ".owl"));
		}

		OWLOntology[] resetOnts = new OWLOntology[reset.length];
		for (int i = 0; i < reset.length; i++) {
			resetOnts[i] = manager.loadOntologyFromOntologyDocument(classloader.getResourceAsStream(reset[i] + ".owl"));
		}
		OWLOntology[] sceneOnts = new OWLOntology[scene.length];
		for (int i = 0; i < scene.length; i++) {
			sceneOnts[i] = manager.loadOntologyFromOntologyDocument(classloader.getResourceAsStream(scene[i] + ".owl"));
		}
