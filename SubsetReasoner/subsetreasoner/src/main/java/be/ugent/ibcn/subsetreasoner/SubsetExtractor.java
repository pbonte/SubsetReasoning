...
//			}
	 ...
	 * defined. Meaning that depth 1 will only retrieve the individuals linked
	 * to the rootInd, whereas depth 2 will also retrieve the individuals linked
	 * to the individuals that are linked to the rootInd, and so on.
	 * 
	 * @param rootInd
	 *            Starting point of the search. The linked individuals to this
	 *            individual will be retrieved
	 * @param ontology
	 *            The ontology in which the axioms from the individuals need to
	 *            be find.
	 * @param depth
	 ...
	 * 
	 * @param rootInd
	 *            Temporarily starting point of the search. The linked
	 *            individuals to this individual will be retrieved
	 * @param ontology
	 *            The ontology in which the axioms from the individuals need to
	 *            be find.
	 * @param depth
	 *            The temporarily depth of the search.
	 * @param axioms
	 *            The list with axioms, we pass it by reference so we don't need
	 *            to create new methodes each time.
	 * @return
	 */
	private void getReferencedAxioms_helper(OWLIndividual rootInd, OWLOntology ontology, int depth,
			Set<OWLAxiom> axioms) {
		...
		// then we loop to find the object properties to add the axioms
		// regarding the linked individual
		for (OWLAxiom ax : ontology.getReferencingAxioms(ind)) {
			if (ax instanceof OWLObjectPropertyAssertionAxiom
				...
				for (OWLClassExpression clsExp : EntitySearcher.getTypes(objInd, ontology)
				...
				for (Entry<OWLDataPropertyExpression, OWLLiteral> ent : EntitySearcher
						.getDataPropertyValues(objInd, ontology).entries()) {
					axioms.add(dfact.getOWLDataPropertyAssertionAxiom(ent.getKey(), objInd, ent.getValue()));
				}
				if (depth - 1 > 0) {
					// if there still is some ground to discover, we dive deeper
					// in the pool with a recursive call
