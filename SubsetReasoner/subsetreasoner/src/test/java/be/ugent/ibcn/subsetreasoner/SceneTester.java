	...
	public void scene7() throws Exception {
		...
		for (int step = 0; step < 10; step++) {
			...
			for (int i = 0; i < setupDelQuery.length; i++) {
				...
				Dataset n = reasoner.queryConstructQuad(setupUpdateQuery[i]);
				Iterator<String> it = n.listNames();
				System.out.println(it.next());
				while(it.hasNext()){
					String name = it.next();
					Model model = n.getNamedModel(name);
					reasoner.addEvent(model, name+setupStreams[i]);
				}
			}
